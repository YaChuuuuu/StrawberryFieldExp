...
ggplot(data = FieldExp109live,mapping = aes(x=菌株 , y = 存活率*100, fill = 菌株)) +
  ...
  labs(title="Survival rate of each treatment",y="Survival rate (%)",fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)
...
ggplot(FieldExp109Grouped_long%>%filter(測量值=="葉綠素"), aes(x = 測量值, y = 平均值, fill = 菌株)) +
  ...
  labs(title="Chlorophyll content of each treatment",y = "Chlorophyll content (SPAD)", fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)
...
ggplot(FieldExp109Grouped_long%>%filter(測量值=="葉面積"), aes(x = 測量值, y = 平均值, fill = 菌株)) +
  ...
  geom_errorbar(aes(ymin = 平均值 - 標準差, ymax = 平均值 + 標準差),
                position = position_dodge(0.8), width = 0.25) +
  labs(title="Leaf area of each treatment",y = "Leaf area (mm^2)", fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)
...
ggplot(FieldExp109Grouped_long%>%filter(測量值=="冠"), aes(x = 測量值, y = 平均值, fill = 菌株)) +
  ...
  geom_errorbar(aes(ymin = 平均值 - 標準差, ymax = 平均值 + 標準差),
                position = position_dodge(0.8), width = 0.25) +
  labs(title="Crown diameter of each treatment",y = "Crown diamete (mm)", fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)

ggplot(FieldExp109Disease, aes(x=菌株 , y = 病害嚴重度, fill = 菌株)) +
  geom_bar(stat = "identity", position = position_dodge(0.8), width = 0.7) +
  geom_text(aes(label = 有病的植株數量), position = position_dodge(0.8), vjust = 1)+ 
  labs(x="Disease severity", fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)
...
ggplot(FieldExp109 %>% filter(!葉綠素_outliers), aes(x = 菌株, y = 葉綠素, fill = 菌株)) +
  geom_boxplot() +
  labs(title="Chlorophyll content of each treatment(w/o outlier)",y = "Chlorophyll content (SPAD)", fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)

ggplot(FieldExp109 %>% filter(!葉面積_outliers), aes(x = 菌株, y = 葉面積, fill = 菌株)) +
  geom_boxplot() +
  labs(title="Leaf area of each treatment(w/o outlier)",y = "Leaf area (mm^2)", fill = "Treatment") +
  theme_minimal() +
...
ggplot(FieldExp109 %>% filter(!冠平均_outliers), aes(x = 菌株, y = 冠平均, fill = 菌株)) +
  geom_boxplot() +
  labs(title="Crown diameter of each treatment(w/o outlier)",y = "Crown diamete (mm)", fill = "Treatment") +
  theme_minimal() +
  facet_grid(場域~時間)
...
tukey_result <- TukeyHSD(anova_result_crown, "菌株")
print(tukey_result)
plot(tukey_result)
'''